...
export const jobTitles = pgTable(
  ...
  (table) => [
    // On `lower(name)`: the service compares titles case-insensitively, so a
    // case-sensitive constraint would let "Trainer" and "trainer" coexist as
    // two rows the application considers one. `code` keeps its plain unique —
    // it is seeded from code, never typed by anyone, and already lower-case.
    uniqueIndex('job_titles_name_lower_uniq').on(sql`lower(${table.name})`),
  ],
);
